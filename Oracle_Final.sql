...
create or replace function FCT_AVG_INDICE_METABO(P_I_NO_ETUDE in varchar2)
  return number
is
  V_INDICE_METABO number(3,2);
begin
  select avg(INDICE_EFFICACITE_METABO_PAT) into V_INDICE_METABO from TP_2_PATIENT
  where NO_PATIENT in (select NO_PATIENT from TP_2_ETUDE_PATIENT where NO_ETUDE = P_I_NO_ETUDE);
  return V_INDICE_METABO;
end;

--select FCT_AVG_INDICE_METABO('747')from DUAL;
